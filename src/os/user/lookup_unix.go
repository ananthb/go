...
func lookupGroup(groupname string) (*Group, error) {
	if client, ok := getUserdbClient(); ok {
		ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
		defer cancel()
		if g, ok, err := client.lookupGroup(ctx, groupname); ok {
			return g, err
		}
	}
...
func lookupGroupId(id string) (*Group, error) {
	if client, ok := getUserdbClient(); ok {
		ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
		defer cancel()
		if g, ok, err := client.lookupGroupId(ctx, id); ok {
			return g, err
		}
	}
	f, err := os.Open(groupFile)
...
func lookupUser(username string) (*User, error) {
	if client, ok := getUserdbClient(); ok {
		ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
		defer cancel()
		if u, ok, err := client.lookupUser(ctx, username); ok {
			return u, err
		}
	}
...
func lookupUserId(uid string) (*User, error) {
	if client, ok := getUserdbClient(); ok {
		ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
		defer cancel()
		if u, ok, err := client.lookupUserId(ctx, uid); ok {
			return u, err
		}
	}
	f, err := os.Open(userFile)
